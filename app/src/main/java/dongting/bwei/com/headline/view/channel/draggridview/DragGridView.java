package dongting.bwei.com.headline.view.channel.draggridview;
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		switch (ev.getAction()) {
			case MotionEvent.ACTION_DOWN:
				...
				System.out.println("--->" + mDragPosition + "<" + getFirstVisiblePosition());
...
//				mStartDragItemView = getChildAt(mDragPosition); //因为gridview会将不可见的item，暂时移除->removeDetachedView
				// 所以不能单纯的用getChildAt(mDragPosition)
...
//				return true;
	...
	private void createDragImage(Bitmap bitmap, int downX, int downY) {
		...
		mWindowLayoutParams.format = PixelFormat.TRANSPARENT; //图片之外的其他地方透明
	...
	private void onSwapItem(int moveX, int moveY) {
		...
		if (tempPosition != mDragPosition && tempPosition != AdapterView.INVALID_POSITION) {
			...
			if (child != null) {//纵向滚动时，不move，不松开，滚动到底时，再反向move一点，就会报nullpointer
				child.setVisibility(View.VISIBLE);//之前的item显示出来
			}
